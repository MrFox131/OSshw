...
HANDLE OpenFile(const char* filename, bool new_file) {
    ...
    HANDLE h;
    if (new_file) {
        h = CreateFile(filename, GENERIC_WRITE, FILE_SHARE_WRITE | FILE_FLAG_OVERLAPPED, NULL, TRUNCATE_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if (h == INVALID_HANDLE_VALUE)
            h = CreateFile(filename, GENERIC_WRITE, FILE_SHARE_WRITE | FILE_FLAG_OVERLAPPED, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
    } else {
